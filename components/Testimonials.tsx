      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-14">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className={`card-float-dark card-glow relative bg-white/5 border border-white/10 rounded-tl-[2.5rem] rounded-br-[2.5rem] rounded-tr-lg rounded-bl-lg p-8 hover:border-white/25 overflow-hidden ${['reveal-left', 'reveal', 'reveal-right'][i] || 'reveal'} reveal-delay-${i + 1} ${isVisible ? 'visible' : ''}`}
            >
              {/* Aspa decorativa */}
              ...
              {/* Estrelas */}
              <div className="flex gap-0.5 mb-4 relative z-10" role="img" aria-label={`${t.stars} de 5 estrelas`}>
                {Array.from({ length: t.stars }).map((_, s) => (
                  <svg key={s} className="w-4 h-4 text-master-gold" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
        ...
        <div className={`text-center reveal reveal-delay-4 ${isVisible ? 'visible' : ''}`}>
          <a
            href="https://pay.hotmart.com/O104206161Q?checkoutMode=10&bid=1775660805600" target="_blank" rel="noopener noreferrer"
